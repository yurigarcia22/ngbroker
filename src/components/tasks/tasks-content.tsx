...
import { LayoutList, LayoutGrid, Plus, Search, Filter, ArrowUpDown } from 'lucide-react'
...
export function TasksContent({ initialTasks, initialStatuses }: { initialTasks: any[], initialStatuses: any[] }) {
    ...
    // SWR Integration with fallback data
    const { data: tasksData, error, mutate, isLoading: swrLoading } = useSWR(
        ...
        },
        {
            ...
            fallbackData: (activeTab === 'my' && activeFilter === 'today' && page === 1 && !debouncedSearch) ? initialTasks : undefined
        }
    )

    const tasks = (tasksData as any[]) || []
    // If we have initial data injected while waiting for SWR, don't show loading spinner
