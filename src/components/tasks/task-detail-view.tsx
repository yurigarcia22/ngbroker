    ...
    const handleToggleTimer = () => {
        if (!task) return
            if (activeTaskId === task.id && isRunning) {
                // Stop logic is usually global via topbar but we can pause/stop here too.
                // Requirement says "Stop" in topbar opens modal. Here maybe just "Pause" or "Focus"?
                // User: "Adicione um botão de ▶️ (Iniciar Cronômetro)... Na topbar botão Parar"
                // So here just start. If running, maybe show "Running" state or allow restart?
                // Let's allow switching task.
            } else {
                startTimer(task.id, task.title)
            }
    }

            const isTaskRunning = activeTaskId === task?.id && isRunning
                ...
                <div className="flex-none px-6 py-3 border-b border-gray-100 flex items-center justify-between bg-white">
                    <div className="flex items-center gap-3">
                        <button
                            ...
                            className={`flex items-center gap-1.5 px-2 py-1 rounded text-xs font-bold transition-all ${isTaskRunning
                                ? 'bg-indigo-100 text-indigo-700 ring-1 ring-indigo-200'
                                : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                                }`}
                            title={isTaskRunning ? "Cronômetro rodando" : "Iniciar cronômetro"}
                        >
                            {isTaskRunning ? (
                                <>
